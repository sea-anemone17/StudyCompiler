import { saveSupabaseConfig, getSupabaseConfig, clearSupabaseConfig } from "./supabaseClient.js";
import { getCurrentUser, signUp, signIn, signOut } from "./auth.js";
import { saveToCloud, loadFromCloud, getLastSyncTime, getCloudSnapshotMeta } from "./cloudSync.js";
...
async function renderSyncInner(container) {
  ...
  const cloudMeta = user ? await getCloudSnapshotMeta().catch(() => null) : null;

  container.innerHTML = `
    <section class="card">
      <h2>Supabase 동기화</h2>
      <p class="muted">localStorage의 Study Compiler 데이터를 Supabase에 JSON 스냅샷으로 저장/복원합니다. 현재 구조는 실시간 병합이 아니라 안전한 백업/복원에 가깝습니다.</p>
      ...
      <input id="syncSupabaseKey" value="${escapeHTML(config.key || "")}" placeholder="eyJ..." />
      <div class="button-row">
        <button id="saveSupabaseConfigBtn">설정 저장</button>
        <button id="clearSupabaseConfigBtn" class="ghost">설정 삭제</button>
      </div>
    </section>
    ...
    <section class="card">
      ...
      <p>클라우드 수정 시각: ${escapeHTML(formatDateTime(cloudMeta?.updated_at))}</p>
      <div class="button-row">
        <button id="saveToCloudBtn">클라우드에 저장</button>
        <button id="loadFromCloudBtn">클라우드에서 불러오기</button>
        <button id="forceLoadFromCloudBtn" class="danger">강제 불러오기</button>
      </div>
      <p class="muted">⚠️ 불러오기는 현재 브라우저 데이터를 클라우드 데이터로 덮어씁니다. 강제 불러오기도 덮어쓰기 전 자동 백업을 남깁니다.</p>
    </section>
  `;
...
function bindSyncEvents(container) {
  ...
  container.querySelector("#loadFromCloudBtn")?.addEventListener("click", async () => {
    try {
      if (!confirm("현재 브라우저 데이터를 클라우드 데이터로 덮어쓸까요? 덮어쓰기 전 자동 백업이 저장됩니다.")) return;
      await loadFromCloud();
  ...
  container.querySelector("#forceLoadFromCloudBtn")?.addEventListener("click", async () => {
    try {
      if (!confirm("충돌 여부와 상관없이 클라우드 데이터로 강제 덮어쓸까요?")) return;
      await loadFromCloud({ force: true });
      alert("클라우드 데이터를 강제로 불러왔습니다.");
      location.reload();
    } catch (error) {
      alert(`강제 불러오기 실패: ${error.message}`);
    }
  });
}
