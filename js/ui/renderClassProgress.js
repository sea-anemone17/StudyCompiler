import {
  CLASS_PROGRESS_TYPES,
  TEACHER_SIGNAL_OPTIONS,
  EXAM_LIKELIHOOD_OPTIONS,
  CLASS_PROGRESS_STUDY_LEVELS
} from "../config.js";
...
export function renderClassProgress(container, state) {
  ...
  container.innerHTML = `
    <section class="card class-progress-card">
      ...
      <p class="hint">시험범위가 아직 애매한 영어/국어는 여기서 먼저 수업 진도를 기록하세요. 기록 강도에 따라 복습만 만들거나, 시험 후보/확정 범위 태스크까지 자동 생성합니다.</p>
      <form id="classProgressForm" class="form-stack class-progress-form">
        ...
        <label for="studyPlanLevel">태스크 생성 강도</label>
        <select id="studyPlanLevel" name="studyPlanLevel">
          ${Object.entries(CLASS_PROGRESS_STUDY_LEVELS).map(([value, label]) => `<option value="${value}" ${value === "reviewOnly" ? "selected" : ""}>${escapeHTML(label)}</option>`).join("")}
        </select>
        <p class="hint">기록만=태스크 없음 · 복습만=당일 복습 · 시험 후보=정리/분석 태스크 생성 · 확정 범위=최종 확인까지 생성</p>
        ...
        <button type="submit" class="accent">진도 기록 + 태스크 생성</button>
      </form>
    </section>
    <section class="card">
...
function renderClassProgressItem(item, subjects) {
  ...
  const level = CLASS_PROGRESS_STUDY_LEVELS[item.studyPlanLevel] || item.studyPlanLevel || "복습만";
  return `
    <article class="class-progress-item ${item.includedInExamRange ? "included" : ""}">
      <div>
        ...
        <small>강조도 ${escapeHTML(signal)} · 시험 가능성 ${escapeHTML(likelihood)} · 생성 강도 ${escapeHTML(level)}</small>
